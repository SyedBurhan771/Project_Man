...
class SoapTaskTransaction(models.Model):
    OPERATION_CREATE = 'create'
    OPERATION_MODIFY = 'modify'
    OPERATION_CHOICES = [
        (OPERATION_CREATE, 'Create'),
    ...
    requested_by = models.ForeignKey(
        ...
        blank=True,
        on_delete=models.SET_NULL,
        related_name='soap_task_transactions',
    )
    ...
    def __str__(self):
        status = 'OK' if self.success else 'ERR'
        return f'SOAP Task {self.operation.upper()} Txn {self.id} [{status}] Task:{self.task_id or "-"} Proj:{self.project_id or "-"}'